...
def test_speed_uses_pace_when_available():
    entries = [
        WorkoutEntry(
            channel=SPORT_CHANNEL,
            day=date(2024, 2, 1),
            workout=WorkoutBlock(pace=5.0, calories=120),
        )
    ]

    records = compute_sport_records(entries)

    assert records.best_speed_kmh == 12
